...
func DivideFood(fc FodderCalculator, quantity int) (float64, error) {
	...
	return amount * factor / float64(quantity), nil
}
...
func ValidateNumberOfCows(quantity int) *InvalidCowsError {
	if quantity > 0 {
		return nil
	} else if quantity == 0 {
		return &InvalidCowsError{0, "no cows don't need food"}
	} else {
		return &InvalidCowsError{quantity, "there are no negative cows"}
	}
}
